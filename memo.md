...
プリコンパイル済みヘッダについて以下が参考になった。
[プリコンパイル済みヘッダーに関するエラーを解決する方法](https://popo2.ara.black/archives/91)

# 新しい描画を作りたい場合のフレームワークの使い勝手について考える

現状、新しい描画表現を試そうとした時にやらなければならないことが複雑過ぎる。
理想の仕組みを考える。

* よく使ういくつかの入力レイアウトは用意されているが、自分で作成することもできる。
* よく使ういくつかの頂点構造は用意されているが、自分で作成することもできる。

## 頂点・インデックス

* 自分で頂点配列を用意し、頂点バッファを作成することができる。
* 頂点バッファの値を書き換えることができる。
* インデックスの仕様はオプションとする。つまり、頂点のみを使い、インデックスは使わないこともできる。

## 定数バッファ

## テクスチャ

## カメラ ( View, Perspective 行列 )
...
Shader =「Mesh を描画するもの（頂点グループのインデックスも指定する）」という作りになっているが、本当にそれで良いか？
Point Sprite を描画したい場合、必ずそのための Mesh が必要になる。描画したいのが単に頂点座標の配列のみで、頂点グループも必要ない場合でも、

# 2024-08-29 Direct3D12 に *SSetShaderResources() はない！

Direct3D11 では、

* VSSetShaderResources() : リソースを VS に設定する
* GSSetShaderResources() : リソースを GS に設定する
* PSSetShaderResources() : リソースを PS に設定する

など、リソースをシェーダーごとに設定する命令があるが、Direct3D12 ではない。
Direct3D12 では、代わりに Root Signature, Descriptor, Descriptor Heap, Descriptor Table といった仕組みを使って、

* D3D12_SHADER_VISIBILITY_VERTEX : VS から使える
* D3D12_SHADER_VISIBILITY_PIXEL : PS から使える
* D3D12_SHADER_VISIBILITY_ALL : すべてのシェーダーステージから使える

といった形で定義する。
複数の値を | して設定することはできなそうなので、

* 特定の１つのシェーダーステージからのみ使える
* すべてのシェーダーステージから使える

のみ設定可能で、

* VS と PS の２つのシェーダーステージから使える

といった設定は行えないよう。

現状の blue-sky の cpp 側のシェーダー定義ファイルは、パラメータごとにどのシェーダーステージに設定を行う仕組みになっているので、Direct3D12 に移行したときに無駄。
( 仮に、「このテクスチャは GS と PS で使う」と blue-sky 側で定義していたとしても、Direct3D12 では「すべてのシェーダーステージで使う」と設定することになる )
Direct3D12 にもうまくフィットするように改良が必要。

=> そもそも、「必要なシェーダーステージにのみリソースを設定した方が無駄がなくパフォーマンスが良いはず」ということで今の仕組みにしたが、本当にそうか？パフォーマンス計測もしていない。
=> とりあえず今はパフォーマンスのことなど気にせず、すべて「すべてのシェーダーステージで使える」としてよいかも。

# 2025-03-02 XAudio2 / DirectSound クリップノイズ検証

* XAudio2 で、シンプルにキーが押されたらサイン波を鳴らすプログラムを作った
    * => キーを連打するとクリップノイズが発生
* blue-sky では DirectSound を使っているが、どうなっているのか？
    * 現状、同じ音を複数回ならすと、ノイズが発生する
        * => これまでそのような状況があまり発生していなかったので、問題にならなかっただけ
